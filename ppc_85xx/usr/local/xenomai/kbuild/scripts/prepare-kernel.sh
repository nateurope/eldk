...
patch_link() {
    ...
    (
		...
		then
                    make_relative_path_link $xenomai_root/$target_dir/$f \
...
make_relative_path_link() {
    tgt=$1
    lnk=$2
    t=`perl -e 'my $tgt="'$tgt'";my $lnk="'$lnk'";'\
'$tgt =~ s#.*/usr/#./#; $lnk =~ s#.*/usr/#./#;'\
'my @tt = split("/","$tgt"); my @ll = split("/","$lnk");'\
'while(scalar(@tt) && scalar(@ll) && ($tt[0] eq $ll[0])){shift @tt;shift @ll;}'\
'print "../"x(scalar(@ll)-1).join("/",@tt);'`
    ln -sf $t $lnk
}
