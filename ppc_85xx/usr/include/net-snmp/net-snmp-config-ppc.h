...
#define NETSNMP_DEFAULT_MIBS ":HOST-RESOURCES-MIB:HOST-RESOURCES-TYPES:UCD-DISKIO-MIB:DISMAN-EVENT-MIB:TCP-MIB:UDP-MIB:MTA-MIB:NETWORK-SERVICES-MIB:IP-MIB:IF-MIB:SNMPv2-MIB:RFC1213-MIB:NOTIFICATION-LOG-MIB:DISMAN-SCHEDULE-MIB:IPV6-ICMP-MIB:IPV6-MIB:IPV6-TCP-MIB:IPV6-UDP-MIB:UCD-SNMP-MIB:UCD-DEMO-MIB:SNMP-TARGET-MIB:NET-SNMP-AGENT-MIB:SNMP-FRAMEWORK-MIB:SNMP-MPD-MIB:SNMP-USER-BASED-SM-MIB:SNMP-VIEW-BASED-ACM-MIB:SNMP-COMMUNITY-MIB:IP-FORWARD-MIB:NET-SNMP-EXTEND-MIB:UCD-DLMOD-MIB:SNMP-NOTIFICATION-MIB:SNMPv2-TM:NET-SNMP-VACM-MIB"
...
#define NETSNMP_CONFIGURE_OPTIONS "'--host=i386-pc-linux-gnu' '--build=i386-pc-linux-gnu' '--target=ppc-denx-linux' '--program-prefix=' '--prefix=/usr' '--exec-prefix=/usr' '--bindir=/usr/bin' '--sbindir=/usr/sbin' '--datadir=/usr/share' '--includedir=/usr/include/net-snmp' '--libdir=/usr/lib' '--libexecdir=/usr/libexec' '--localstatedir=/var' '--sharedstatedir=/usr/com' '--mandir=/usr/share/man' '--infodir=/usr/share/info' '--host=ppc-linux' '--with-endianness=big' '--enable-static' '--enable-shared' '--with-cflags=-O2 -g -fsigned-char' '--with-ldflags= -lcrypto' '--with-sys-location=Unknown' '--with-logfile=/var/log/snmpd.log' '--with-persistent-directory=/var/net-snmp' '--with-mib-modules=host agentx smux		    ucd-snmp/diskio disman/event disman/schedule tcp-mib udp-mib mibII/mta_sendmail' '--with-libwrap' '--sysconfdir=/etc' '--enable-ipv6' '--enable-ucd-snmp-compatibility' '--with-openssl' '--with-pic' '--disable-embedded-perl' '--with-perl-modules=no' '--enable-mfd-rewrites' '--enable-local-smux' '--with-sys-contact=root@localhost' 'CFLAGS=-O2 -g -fsigned-char' 'CXXFLAGS=-O2 -g -fsigned-char' 'FFLAGS=-O2 -g -fsigned-char' 'build_alias=i386-pc-linux-gnu' 'host_alias=ppc-linux' 'target_alias=ppc-denx-linux'"
