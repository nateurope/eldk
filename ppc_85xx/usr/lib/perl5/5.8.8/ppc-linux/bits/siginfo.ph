...
if((!defined (defined(&__have_siginfo_t) ? &__have_siginfo_t : undef)  && (defined (defined(&_SIGNAL_H) ? &_SIGNAL_H : undef) || defined (defined(&__need_siginfo_t) ? &__need_siginfo_t : undef)))) {
    ...
    eval 'sub si_timer1 () { ($_sifields->{_timer}->{_timer1});}' unless defined(&si_timer1);
    eval 'sub si_timer2 () { ($_sifields->{_timer}->{_timer2});}' unless defined(&si_timer2);
    ...
    eval("sub SI_ASYNCNL () { -6; }") unless defined(&SI_ASYNCNL);
